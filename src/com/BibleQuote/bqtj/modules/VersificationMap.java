...
// поддержка с Java 6; Android API 9 (Android 2.3.1)
//import java.util.TreeMap;

// поддержка для Java 5; Android API <9 (Android <2.3.1)
...
public class VersificationMap {
	...
	public class VerseDifferences {
		...
		public VerseDifferences(int iDifCh, int iDifVs, int iVsRepeated,
										int iCountSequence, int iNextChapter) {
			this.iDifCh = iDifCh;
			this.iDifVs = iDifVs;
	...
	private class TreeMapBackport {
		...
		public Map.Entry floorEntry(Object key) {
			...
			if (treeMap != null) {
				edu.emory.mathcs.backport.java.util.TreeMap tm
						= (edu.emory.mathcs.backport.java.util.TreeMap) treeMap;
			...
			try {
				...
				while (xppVersMap.getEventType() != XmlPullParser.END_DOCUMENT) {
					switch (xppVersMap.getEventType()) {
						...
						//	Lod.d(LOG_TAG, "text = " + xppVersMap.getText());
						//	break;
